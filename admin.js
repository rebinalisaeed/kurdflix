...
document.getElementById('addMovieForm')?.addEventListener('submit', (e) => {
    ...
    if (editingMovieId) {
        ...
        if (index !== -1) {
            currentData.movies[index] = { ...currentData.movies[index], ...movieData };
...
function renderStats() {
    ...
    container.innerHTML = `
        <div class="form-row-3">
            ...
            <div style="background-color: var(--accent); padding: 1rem; border-radius: 8px; text-align: center;"><h3>${currentData.carousel.length}</h3><p>سلاید</p></div>
        </div>
    `;
}
